...
//
// Usage:
//   reduce_bench --variant baseline --n 1048576 --iters 200 --warmup 20
//
...
int main(int argc, char** argv) {
    ...
    const double bytes_read = static_cast<double>(n) * sizeof(float);
    const double bw_gb_s    = bytes_read / (static_cast<double>(avg_ms) * 1e-3) / 1e9;

    std::fprintf(stdout, "time_ms=%.4f\n", avg_ms);
    std::fprintf(stderr,
        "reduce_bench: n=%d  variant=%d  iters=%d warmup=%d\n"
