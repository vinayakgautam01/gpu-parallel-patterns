...
int main(int argc, char** argv) {
    ...
    if (n <= 0) {
        std::fprintf(stderr, "scan_cpu_timing: n must be > 0 (got %d)\n", n);
        return 1;
    }

    std::vector<float> h_in(static_cast<size_t>(n));
    std::vector<float> h_out(static_cast<size_t>(n), 0.0f);
    ...
    for (int i = 0; i < cfg.warmup; ++i) {
        gpp::scan::inclusive_scan_cpu_ref(h_in.data(), h_out.data(), n);
        sink += h_out[static_cast<size_t>(n - 1)];
    }
    ...
    std::fprintf(stderr,
        "scan_cpu_timing: n=%d iters=%d warmup=%d\n"
