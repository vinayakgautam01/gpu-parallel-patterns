...
def load_csv(path: Path) -> pd.DataFrame:
    ...
    df = df.dropna(subset=["variant", "I", "time_ms"]).copy()
...
def save_plot_time_vs_n(df: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.yscale("log")
    plt.xlabel("N (matrix side length, NxNxN)")
...
def save_plot_speedup_vs_n(df: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.title("GEMM: speedup vs baseline")
...
def write_markdown(cfg: Config, df_agg: pd.DataFrame, csv_path: Path) -> None:
    ...
    time_plot = cfg.out_dir / "gemm_time.png"
    speed_plot = cfg.out_dir / "gemm_speedup.png"
    ...
    md.append("> Notes:\n")
    md.append(
        "> - `cpu_ref` is the single-threaded CPU reference "
        "(not a GPU variant).\n"
    )
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        description=(
            "Generate GEMM benchmark markdown + plots from latest CSV."
        )
    )
    ap.add_argument(
        "--csv",
        ...
        default=None,
        help=(
            "Path to CSV. If omitted, auto-picks latest gemm_*.csv "
            "in benchmarks/results/."
        ),
    )
    pat_dir = root / "docs" / "plots" / "gemm"
    ap.add_argument(
        "--out-md",
        type=str,
        default=str(pat_dir / "gemm_results.md"),
    ...
    ap.add_argument(
        "--title",
        type=str,
        default="GEMM Benchmark Results",
        help="Markdown title.",
    )
    args = ap.parse_args()
    ...
    if cfg.csv:
        ...
        csv_path = latest_file(results_dir, "gemm_*.csv")
        if csv_path is None:
            csv_path = latest_file(results_dir, "bench_*.csv")

    if csv_path is None or not csv_path.exists():
        raise SystemExit(
            "No benchmark CSV found. Run: bash scripts/bench_gemm.sh"
        )
