...
def save_plot_time_vs_n(df: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.title("Histogram: time vs input size")
    plt.grid(True)
    plt.legend()
...
def save_plot_speedup_vs_n(df: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.title("Histogram: speedup vs baseline")
...
def write_markdown(cfg: Config, df_agg: pd.DataFrame, csv_path: Path) -> None:
    ...
    time_plot = cfg.out_dir / "hist_time.png"
    speed_plot = cfg.out_dir / "hist_speedup.png"
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        description=(
            "Generate histogram benchmark markdown + plots from latest CSV."
        )
    )
    ap.add_argument(
        "--csv",
        ...
        default=None,
        help=(
            "Path to CSV. If omitted, auto-picks latest hist_*.csv "
            "in benchmarks/results/."
        ),
    )
    pat_dir = root / "docs" / "plots" / "histogram"
    ap.add_argument(
        "--out-md",
        type=str,
        default=str(pat_dir / "histogram_results.md"),
    ...
    ap.add_argument(
        "--title",
        type=str,
        default="Histogram Benchmark Results",
        help="Markdown title.",
    )
    args = ap.parse_args()
    ...
    if cfg.csv:
        ...
        csv_path = latest_file(results_dir, "hist_*.csv")
        if csv_path is None:
            csv_path = latest_file(results_dir, "bench_*.csv")

    if csv_path is None or not csv_path.exists():
        raise SystemExit("No benchmark CSV found. Run: bash scripts/bench_hist.sh")
