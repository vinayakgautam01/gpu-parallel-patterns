...
# Histogram-specific benchmark sweep.
#
# Usage:
#   ./scripts/bench_hist.sh
#   HIST_VARIANTS="baseline opt3 opt4" ./scripts/bench_hist.sh
#   HIST_SIZES="1048576 16777216 268435456" ./scripts/bench_hist.sh
...
OUT_CSV="${RESULTS_DIR}/hist_${TIMESTAMP}.csv"
OUT_ENV="${RESULTS_DIR}/hist_${TIMESTAMP}_env.txt"

# ---------------------------
# Knobs (override via env vars)
# ---------------------------

if [[ -n "${HIST_VARIANTS:-}" ]]; then
  read -r -a VARIANTS <<< "${HIST_VARIANTS}"
else
  VARIANTS=(baseline opt1 opt2 opt3 opt4)
fi

if [[ -n "${HIST_SIZES:-}" ]]; then
  read -r -a SIZES <<< "${HIST_SIZES}"
else
  SIZES=(1024 4096 16384 65536 262144 1048576 4194304 16777216 67108864 134217728 268435456)
fi

FIXED_ITERS="${HIST_ITERS:-}"
FIXED_WARMUP="${HIST_WARMUP:-}"
