...
def load_csv(path: Path) -> pd.DataFrame:
    ...
    if "eff_bw_gbs" in df.columns:
        df["eff_bw_gbs"] = pd.to_numeric(df["eff_bw_gbs"], errors="coerce")
...
def save_plot_time_vs_n(df: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.title("Scan: time vs input size")
    plt.grid(True)
    plt.legend()
...
def save_plot_speedup_vs_n(df: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.title("Scan: speedup vs baseline")
...
def write_markdown(cfg: Config, df_agg: pd.DataFrame, csv_path: Path) -> None:
    ...
    time_plot = cfg.out_dir / "scan_time.png"
    speed_plot = cfg.out_dir / "scan_speedup.png"
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        description=(
            "Generate scan benchmark markdown + plots from latest CSV."
        )
    )
    ap.add_argument(
        "--csv",
        ...
        default=None,
        help=(
            "Path to CSV. If omitted, auto-picks latest scan_*.csv "
            "in benchmarks/results/."
        ),
    )
    pat_dir = root / "docs" / "plots" / "scan"
    ap.add_argument(
        "--out-md",
        type=str,
        default=str(pat_dir / "scan_results.md"),
    ...
    if cfg.csv:
        ...
        csv_path = latest_file(results_dir, "scan_*.csv")
        if csv_path is None:
            csv_path = latest_file(results_dir, "bench_*.csv")

    if csv_path is None or not csv_path.exists():
        raise SystemExit(
            "No benchmark CSV found. Run: bash scripts/bench_scan.sh"
        )
