...
def save_plot_time_vs_n(df: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.title("Reduce: time vs input size")
    plt.grid(True)
    plt.legend()
...
def save_plot_speedup_vs_n(df: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.title("Reduce: speedup vs baseline")
...
def write_markdown(cfg: Config, df_agg: pd.DataFrame, csv_path: Path) -> None:
    ...
    time_plot = cfg.out_dir / "reduce_time.png"
    speed_plot = cfg.out_dir / "reduce_speedup.png"
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        description=(
            "Generate reduce benchmark markdown + plots from latest CSV."
        )
    )
    ap.add_argument(
        "--csv",
        ...
        default=None,
        help=(
            "Path to CSV. If omitted, auto-picks latest reduce_*.csv "
            "in benchmarks/results/."
        ),
    )
    pat_dir = root / "docs" / "plots" / "reduce"
    ap.add_argument(
        "--out-md",
        type=str,
        default=str(pat_dir / "reduce_results.md"),
    ...
    ap.add_argument(
        "--title",
        type=str,
        default="Reduce Benchmark Results",
        help="Markdown title.",
    )
    args = ap.parse_args()
    ...
    if cfg.csv:
        ...
        csv_path = latest_file(results_dir, "reduce_*.csv")
        if csv_path is None:
            csv_path = latest_file(results_dir, "bench_*.csv")

    if csv_path is None or not csv_path.exists():
        raise SystemExit(
            "No benchmark CSV found. Run: bash scripts/bench_reduce.sh"
        )
