...
def save_plot_time_vs_n(df: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.yscale("log")
    plt.xlabel("n (total merged elements = m + n)")
...
def save_plot_speedup_vs_n(df: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.title("Merge: speedup vs baseline")
...
def write_markdown(cfg: Config, df_agg: pd.DataFrame, csv_path: Path) -> None:
    ...
    time_plot = cfg.out_dir / "merge_time.png"
    speed_plot = cfg.out_dir / "merge_speedup.png"
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        description=(
            "Generate merge benchmark markdown + plots from latest CSV."
        )
    )
    ap.add_argument(
        "--csv",
        ...
        default=None,
        help=(
            "Path to CSV. If omitted, auto-picks latest merge_*.csv "
            "in benchmarks/results/."
        ),
    )
    pat_dir = root / "docs" / "plots" / "merge"
    ap.add_argument(
        "--out-md",
        type=str,
        default=str(pat_dir / "merge_results.md"),
    ...
    ap.add_argument(
        "--title",
        type=str,
        default="Merge Benchmark Results",
        help="Markdown title.",
    )
    args = ap.parse_args()
    ...
    if cfg.csv:
        ...
        csv_path = latest_file(results_dir, "merge_*.csv")
        if csv_path is None:
            csv_path = latest_file(results_dir, "bench_*.csv")

    if csv_path is None or not csv_path.exists():
        raise SystemExit(
            "No benchmark CSV found. Run: bash scripts/bench_merge.sh"
        )
